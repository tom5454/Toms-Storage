...
public class InventoryConfiguratorScreen extends TSContainerScreen<InventoryConfiguratorMenu> {
	private static final Identifier GUI_TEXTURES = Identifier.tryBuild(StorageMod.modid, "textures/gui/inventory_configurator.png");
	...
	protected void init() {
		...
		buttonAddConnected = addRenderableWidget(new IconButton(leftPos - 18, topPos + 5 + 18, Component.translatable("narrator.toms_storage.add"), Identifier.tryBuild(StorageMod.modid, "icons/add"), () -> {
			click(1);
		}));
		buttonAddConnected.setTooltip(Tooltip.create(Component.translatable("tooltip.toms_storage.inv_config.add_connected")));

		buttonRemoveAll = addRenderableWidget(new IconButton(leftPos - 18, topPos + 5 + 18 * 2, Component.translatable("narrator.toms_storage.removeAll"), Identifier.tryBuild(StorageMod.modid, "icons/deny"), () -> {
			click(2);
		}));
		...
		buttonSkip = addRenderableWidget(ToggleButton.builder(leftPos - 18, topPos + 5 + 18 * 4).
				...
				iconOff(Identifier.tryBuild(StorageMod.modid, "icons/include_inv")).
				iconOn(Identifier.tryBuild(StorageMod.modid, "icons/skip_inv")).
				build(s -> {
					click(((s ? 1 : 0) << 3) | 4);
				}));
		...
		buttonKeepLast = addRenderableWidget(ToggleButton.builder(leftPos - 18, topPos + 5 + 18 * 5).
				...
				iconOn(Identifier.tryBuild(StorageMod.modid, "icons/keep_last_1")).
				build(s -> {
					click(((s ? 1 : 0) << 3) | 5);
				}));
		...
		buttonRemoveFilter = addRenderableWidget(new IconButton(leftPos + imageWidth - 22, topPos - 22, Component.translatable("narrator.toms_storage.removeFilter"), Identifier.tryBuild(StorageMod.modid, "icons/deny"), () -> {
			click(7);
		}));
