...
public abstract class AbstractStorageTerminalScreen<T extends StorageTerminalMenu> extends AbstractContainerScreen<T> implements IDataReceiver {
	...
	protected void init() {
		...
		super.init();
		this.searchField = new EditBox(font, this.leftPos + 82, this.topPos + 6, 89, this.font.lineHeight, Component.translatable("narrator.toms_storage.terminal_search"));
	...
	public void render(PoseStack matrices, int mouseX, int mouseY, float partialTicks) {
		boolean flag = GLFW.glfwGetMouseButton(mc.getWindow().getWindow(), GLFW.GLFW_MOUSE_BUTTON_LEFT) != GLFW.GLFW_RELEASE;
		int i = this.leftPos;
		int j = this.topPos;
		int k = i + 174;
		int l = j + 18;
		...
		j = l;
		k = j1;
		blit(matrices, i, j + (int) ((k - j - 17) * this.currentScroll), 232 + (this.needsScrollBars() ? 0 : 12), 0, 12, 15);
		matrices.pushPose();
		...
		if (buttonSortingType.isHoveredOrFocused()) {
			renderTooltip(matrices, Component.translatable("tooltip.toms_storage.sorting_" + buttonSortingType.state), mouseX, mouseY);
		}
		if (buttonSearchType.isHoveredOrFocused()) {
			renderTooltip(matrices, Component.translatable("tooltip.toms_storage.search_" + buttonSearchType.state, "REI"), mouseX, mouseY);
		}
		if (buttonCtrlMode.isHoveredOrFocused()) {
			renderComponentTooltip(matrices, Arrays.stream(I18n.get("tooltip.toms_storage.ctrlMode_" + buttonCtrlMode.state).split("\\\\")).map(Component::literal).collect(Collectors.toList()), mouseX, mouseY);
		}
	}
	...
	public void drawSlot(PoseStack st, SlotStorage slot, int mouseX, int mouseY) {
	...
	public boolean drawTooltip(PoseStack st, SlotStorage slot, int mouseX, int mouseY) {
		if (slot.stack != null) {
	...
	public final void renderItemInGui(PoseStack st, ItemStack stack, int x, int y, int mouseX, int mouseY, boolean hasBg, int color, boolean tooltip, String... extraInfo) {
		if (stack != null) {
			if (!tooltip) {
				...
				this.itemRenderer.renderGuiItemDecorations(font, stack, x, y);
	...
	protected void renderBg(PoseStack st, float partialTicks, int mouseX, int mouseY) {
		...
		RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
		RenderSystem.setShaderTexture(0, getGui());
		blit(st, this.leftPos, this.topPos, 0, 0, this.imageWidth, this.imageHeight);
	}
	...
	public void receive(CompoundTag tag) {
		menu.receiveClientTagPacket(tag);
		refreshItemList = true;
	}
