...
public class TerminalSyncManager {
	...
	private void write(PacketByteBuf buf, StoredItemStack stack) {
		ItemStack st = stack.getStack();
		Item item = st.getItem();
		NbtCompound NbtCompound = getSyncTag(st);
		byte flags = (byte) ((stack.getQuantity() == 0 ? 1 : 0) | (NbtCompound != null ? 2 : 0));
		boolean wr = true;
	...
	private StoredItemStack read(PacketByteBuf buf) {
		...
		if(rd) {
			stack = new StoredItemStack(new ItemStack(Item.byRawId(buf.readVarInt())));
		} else {
	...
	public void update(Map<StoredItemStack, Long> items, ServerPlayerEntity player, Consumer<NbtCompound> extraSync) {
	...
	public void receiveInteract(NbtCompound tag, InteractHandler handler) {
		if(tag.contains("a")) {
			PacketByteBuf buf = new PacketByteBuf(Unpooled.wrappedBuffer(tag.getByteArray("a")));
			byte flags = buf.readByte();
			StoredItemStack stack;
			if((flags & 2) != 0) {
				stack = null;
