...
- Filtered connectors
- Inventory Hopper
- **Smart Insertion**: Intelligently organizes item insertion by prioritizing existing stacks and fuller containers to optimize storage organization.
- **Inventory Consolidation**: A new button in the Storage Terminal that merges scattered stacks of the same item into the fullest available containers.

### Smart Insertion Addon
Improves the storage logic by intercepting item insertion to keep your chests organized:
1. **Same Item First**: Prioritizes containers that already contain the inserted item.
2. **Item Families**: If the item is new, it looks for containers with "similar" items using Minecraft/Common Tags (e.g., `#c:ingots`, `#minecraft:logs`).
3. **Stack Optimization**: Completes partial stacks before using new empty slots.
4. **Original Fallback**: Maintains original mod priority logic if no better candidate is found.

*Note: Can be disabled in the mod configuration menu.*
